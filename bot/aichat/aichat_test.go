...
// TestAIChat_GenerationOptions_OmitStopWords verifies that requests remain
// compatible with models that reject the stop parameter.
func TestAIChat_GenerationOptions_OmitStopWords(t *testing.T) {
	var options llms.CallOptions
	for _, option := range generationOptions(0.7, 80) {
		option(&options)
	}

	if len(options.StopWords) != 0 {
		t.Fatalf("generation options include unsupported stop sequences: %q", options.StopWords)
	}
	if options.MaxTokens != 80 {
		t.Errorf("MaxTokens = %d, want 80", options.MaxTokens)
	}
}
